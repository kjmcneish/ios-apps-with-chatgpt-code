...
class RestaurantCollectionViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, RestaurantEditDelegate, RestaurantDirectionsDelegate {
    ...
    @IBAction func filterButtonTapped(_ sender: Any) {
        ...
        let clearFilterAction = UIAlertAction(title: "Clear Filter", style: .default, handler: { _ in
            self.restaurants = Restaurant.shared.getAllEntities() // Reset
    ...
    @IBAction func sortButtonTapped(_ sender: Any) {
        ...
        let cuisineAction = UIAlertAction(title: "Cuisine", style: .default, handler: { _ in
            ...
            self.sortRestaurants() // Use the refactored method
        })
    ...
    // Method to handle sorting restaurants based on the current sort option
    func sortRestaurants() {
        ...
        case .distance:
            self.restaurants.sort {
                guard let lat1 = $0.latitude, let lon1 = $0.longitude else { return false }
                guard let lat2 = $1.latitude, let lon2 = $1.longitude else { return true }
    ...
    func didTapDirections(for restaurant: RestaurantEntity) {
        if let latitude = restaurant.latitude, let longitude = restaurant.longitude {
            openAppleMaps(restaurantName: restaurant.name, latitude: latitude, longitude: longitude)
        }
        else if restaurant.fullAddress != "No address specified" {
            // Use geocoding to convert the address to coordinates
            let geocoder = CLGeocoder()
            geocoder.geocodeAddressString(restaurant.fullAddress) { placemarks, error in
                if error != nil {
                    // Display a user-friendly message if geocoding fails
                    let alert = UIAlertController(title: "Address Not Found",
                       message: "We couldn't find the location for the address provided. Please try again later.",
                ...
                if let placemark = placemarks?.first, let location = placemark.location {
                    ...
                    // Open the maps with the retrieved coordinates
                    self.openAppleMaps(restaurantName: restaurant.name, latitude: latitude, longitude: longitude)
                }
            }
        }
    }
