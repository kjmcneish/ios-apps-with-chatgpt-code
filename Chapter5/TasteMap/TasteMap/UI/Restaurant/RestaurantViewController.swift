...
class RestaurantViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, RestaurantCellDelegate {
	...
	func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		...
		if let latitude = restaurantEntity.latitude, let longitude = restaurantEntity.longitude {
		...
		if let cuisineName = restaurantEntity.cuisine?.name {
			if let distance = distanceText {
				cell.lblCuisine.text = "\(cuisineName) - \(distance)"
			} else {
				cell.lblCuisine.text = cuisineName
			}
		} else {
			if let distance = distanceText {
				cell.lblCuisine.text = distance
			} else {
				cell.lblCuisine.text = nil // or an empty string, depending on your UI preference
			}
		}
		...
		// Create an attributed string with default black color
        ...
        cell.photos = photos
        cell.imageCollectionView.reloadData() // Reload the internal collection view
